...
## InferenceAmiVersion — the silent killer

Recent vLLM DLC tags (anything CUDA 13+, including the current default `0.21.0-...-cu130-...`) **require** setting `InferenceAmiVersion` on the SageMaker ProductionVariant. Without it, SageMaker may land the container on an older host AMI with incompatible CUDA drivers, and the container dies during initialization.

**The failure signature is brutal:**
- `CannotStartContainerError` after ~10–15 minutes
- **No CloudWatch log group ever created**
- Identical symptoms across image versions, instance families, IAM roles, env vars

The CUDA/driver mismatch breaks initialization before the container's logging is even up, so there's nothing to look at. Agents (and humans) routinely misdiagnose this as a quota issue, a VPC issue, a model-incompatibility issue, or an account-level problem — none of which it is.

**How to get the right AMI:** use `--format json` on the resolver. The script parses the CUDA version out of the image tag and returns the required AMI:

```bash
python <skill-path>/scripts/resolve_image_uri.py --family vllm --region eu-west-1 --format json
# {"image_uri": "763104351884.dkr.ecr.eu-west-1.amazonaws.com/vllm:0.21.0-...-cu130-...",
#  "inference_ami_version": "al2-ami-sagemaker-inference-gpu-3-1"}
```

Pass the resulting `inference_ami_version` to `deploy.py --inference-ami-version <value>`. If the resolver returns `null` for `inference_ami_version`, the image's CUDA version doesn't need an override and SageMaker's default AMI is fine — pass `null`/omit the flag.

**Rule of thumb**: any time the image tag contains `cu130` or later, you must set `InferenceAmiVersion`. The default URI-only mode of the resolver prints a warning to stderr when this applies, but the `--format json` mode is the right way to wire it through programmatically.
