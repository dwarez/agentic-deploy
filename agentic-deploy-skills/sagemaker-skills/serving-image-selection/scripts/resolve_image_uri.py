...
def resolve_hf_inference(region: str) -> str:
    """HuggingFace Inference Toolkit — for embeddings, classifiers, non-LLM models."""
    try:
        from sagemaker import image_uris
    except ImportError:
        raise SystemExit("sagemaker SDK not installed — run python-env-setup")
...
def main() -> int:
    ...
    # DJL-LMI and HF Inference use SDK helpers that select compatible AMIs internally
    ami = None
