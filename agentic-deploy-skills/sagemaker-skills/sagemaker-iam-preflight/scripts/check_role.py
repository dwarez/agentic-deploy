...
def main() -> int:
    ...
    ranked: list[tuple[str, str]] = []  # (last_used_sort_key, role_name)
    for name in candidates:
        ...
        ranked.append((last_used, name))

    ranked.sort(reverse=True)

    log("Ranking (most recent first):")
    for last_used, name in ranked:
        log(f"  - {name} ({'last used ' + last_used if last_used else 'never used'})")
