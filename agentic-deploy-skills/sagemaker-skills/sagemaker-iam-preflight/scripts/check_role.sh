...
# Rank candidates by RoleLastUsed.LastUsedDate (most recent first).
#
# Why: in accounts with multiple SageMaker roles (auto-generated +
# manually-created + project-specific), the alphabetically-first role is
# rarely the actively-maintained one. The role that was assumed most
# recently is the most likely to have current policies (e.g. cross-account
# ECR pull for the AWS public DLC) and to actually work.
#
# Roles never used get a placeholder timestamp of "0" so they sort last.
log "Ranking candidates by last-used date..."
...
for candidate in "${CANDIDATES[@]}"; do
    ...
    if [[ "$LAST_USED" == "None" || -z "$LAST_USED" ]]; then
        LAST_USED="0"  # never-used roles sort last
    fi
    RANKED+=("${LAST_USED}|${candidate}")
done

# Sort descending by timestamp (ISO 8601 sorts correctly as a string)
IFS=$'\n' SORTED=($(printf '%s\n' "${RANKED[@]}" | sort -r))
unset IFS

# Log the ranking so the user can see what we picked and why
...
# Validate in ranked order, return the first one that works
for entry in "${SORTED[@]}"; do
    candidate="${entry##*|}"
