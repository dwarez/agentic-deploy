...
Usage (minimal):
    python deploy.py \\
        ...
        --image-uri 763104351884.dkr.ecr.eu-west-1.amazonaws.com/vllm:0.11.2-sagemaker-v1.2 \\
...
def wait_for_endpoint(sm: Any, endpoint_name: str, timeout_minutes: int = 30) -> None:
    log(
        f"Waiting for endpoint {endpoint_name} to reach InService (up to {timeout_minutes} min)..."
    )
    start = time.time()
    ...
    raise TimeoutError(
        f"Endpoint did not reach InService within {timeout_minutes} minutes"
    )
...
def main() -> int:
    ...
    p.add_argument(
        "--model-name",
        required=True,
        help="SageMaker model name (also used as base for endpoint name)",
    )
    p.add_argument(
        "--image-uri",
        required=True,
        help="Serving container image URI (from serving-image-selection)",
    )
    p.add_argument(
        "--role-arn",
        required=True,
        help="SageMaker execution role ARN (from sagemaker-iam-preflight)",
    )
    ...
    p.add_argument(
        "--model-s3-uri",
        default=None,
        help="S3 URI to model artifacts (omit when loading from HF Hub)",
    )
    p.add_argument(
        "--env",
        action="append",
        default=[],
        help="Container env var, KEY=VALUE. Repeatable.",
    )
    ...
    p.add_argument(
        "--endpoint-name",
        default=None,
        help="Override default endpoint name (default: <model-name>-<timestamp>)",
    )
    p.add_argument(
        "--project", default=None, help="Tag value for 'Project' (default: model name)"
    )
    p.add_argument(
        "--environment",
        default=DEFAULTS["environment_tag"],
        help=f"Tag value for 'Environment' (default: {DEFAULTS['environment_tag']})",
    )
    ...
    p.add_argument(
        "--no-autoscaling",
        action="store_true",
        help="Skip autoscaling registration (NOT RECOMMENDED)",
    )
    ...
    p.add_argument(
        "--enable-data-capture",
        action="store_true",
        help="Enable request/response logging to S3 (off by default)",
    )
    p.add_argument(
        "--data-capture-s3-uri",
        default=None,
        help="S3 URI for data capture (default: s3://sagemaker-<region>-<account>/<endpoint>/data-capture/)",
    )
    ...
    # AWS
    p.add_argument(
        "--region", required=True, help="AWS region (from aws-context-discovery)"
    )
    ...
    if not args.no_autoscaling:
        ...
        log(
            "WARNING: autoscaling skipped per --no-autoscaling. Endpoint will not scale with traffic."
        )
    ...
    log(
        f"  Data capture:    {args.data_capture_s3_uri if args.enable_data_capture else 'OFF (pass --enable-data-capture to turn on)'}"
    )
    log("")
    log("Test invocation:")
