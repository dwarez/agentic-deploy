...
The DLC is published to AWS-managed regional ECR repositories. There is no public SageMaker SDK helper for this image specifically (as of the skill write date), so look up the URI from the DLC release page:
...
```
763104351884.dkr.ecr.<region>.amazonaws.com/vllm:<vllm-version>-sagemaker-v<integration-version>
```

The account ID `763104351884` is the AWS public DLC account for `us-east-1` and `us-west-2`. **Other regions use different account IDs** — confirm against the DLC docs. This is a frequent source of "ImagePullError" failures.
...
The container reads model and inference configuration from environment variables. Common ones:
